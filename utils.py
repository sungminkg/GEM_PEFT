...
def forward_wrap_with_option_len(self, input_ids=None, labels=None, option_len=None, num_options=None, return_dict=None,
                                 **kwargs):
    """
...
def encode_prompt_train(task, template, train_samples, eval_sample, tokenizer, max_length, sfc=False, icl_sfc=False,
...
def encode_prompt_eval(task, template, eval_samples, tokenizer, max_length, sfc=False, icl_sfc=False,
                       generation=False, max_new_tokens=None):
    """
    Encode prompts for eval_sample
    Input: 
    - task, template: task and template class
    ...
    for eval_sample in eval_samples:
        ...
        if not generation:
            ...
            verbalized_eval_prompts = [verbalize_fn(eval_sample, cand).strip(' ') for cand in eval_sample.candidates]
            unverbalized_eval_prompt_length = len(tokenizer.encode(unverbalized_eval_prompt))
            option_lens.extend([(len(tokenizer.encode(verbalized_eval_prompt)) - unverbalized_eval_prompt_length) for
                                verbalized_eval_prompt in verbalized_eval_prompts])

            # With demonstrations
            final_prompts.extend([eval_prompt.lstrip().strip(' ') for eval_prompt in
                                  verbalized_eval_prompts])
        else:
            assert not sfc and not icl_sfc, "Generation tasks do not support SFC"
            option_lens.extend([0])
            final_prompts.extend([unverbalized_eval_prompt.lstrip().strip(' ')])
    ...
    # logger.info(f'attention_masks length: {len(attention_masks)}')
