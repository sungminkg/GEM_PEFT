        ...
        for name, module in self.model.named_modules():
            if isinstance(module, EntropyMaskingLinear) and any(layer_name in name for layer_name in self.linear_module_list):
                ...
                if grad_tensor is not None and weight_tensor is not None:
                    ...
                    # entropy_score = layer_norm * (1 + entropy / torch.log(torch.tensor(ratio.numel(), dtype=torch.float32)))
                    # entropy_score = layer_norm
                    entropy_score = layer_norm * entropy
                    
                    # lam = 0.5
                    # entropy_score = (layer_norm ** lam) * (entropy ** (1 - lam))
                    
                    # gini = compute_gini(prob)
                    # entropy_score = layer_norm * gini
...
def compute_gini(prob_tensor: torch.Tensor) -> float:
    """Compute Gini coefficient from a probability distribution tensor."""
    prob_flat = prob_tensor.view(-1)
    prob_sorted, _ = torch.sort(prob_flat)
    n = prob_sorted.numel()
    index = torch.arange(1, n + 1, dtype=prob_sorted.dtype, device=prob_sorted.device)
    gini = (2 * (index * prob_sorted).sum() / prob_sorted.sum() - (n + 1)) / n
    return gini.item()
