...
class GradWeightLayerNormMaskingLinear(nn.Module):
    ...
    def apply_mask(self, k, mask_mode, grad_tensor: torch.Tensor, weight_tensor: torch.Tensor, 
                   layer_grad_norm: torch.Tensor, layer_weight_norm: torch.Tensor): 
        grad_ratio = grad_tensor / (layer_grad_norm + 1e-8)
        weight_ratio = weight_tensor / (layer_weight_norm + 1e-8)
        ratio = grad_ratio / (weight_ratio + 1e-8)  # (parameter gradient / layer gradient norm) / (parameter weight / layer weight norm)

        # Compute top-k threshold based on masking probability
        if k > 0:
...
class GradWeightLayerNormMasking:
    def __init__(self, model, mask_mode, gradients, weights, masking_prob):
        ...
        for key, module in self.model.named_modules():
            if any(key.endswith(layer_name) for layer_name in self.linear_module_list) and isinstance(module, nn.Linear):
                ...
                setattr(parent_module, sub_key, 
                        GradWeightLayerNormMaskingLinear(base_Linear=module, masking_prob=self.masking_prob))
        ...
        self.apply_grad_weight_ln_masking()
    ...
    def apply_grad_weight_ln_masking(self):
        for name, module in self.model.named_modules():
            if isinstance(module, GradWeightLayerNormMaskingLinear) and any(layer_name in name for layer_name in self.linear_module_list):
                ...
                if grad_tensor is not None and weight_tensor is not None:
                    ...
                    print(f'layer grad norm: {layer_grad_norm}\tlayer weight norm: {layer_weight_norm}')
                    k = self._tuning_params_per_module()
                    module.apply_mask(k, self.mask_mode, grad, weight, layer_grad_norm, layer_weight_norm)
