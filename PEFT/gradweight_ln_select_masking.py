...
# class GradWeightlnSelectMaskingLinear(nn.Module):
...
#         grad_ratio = grad_tensor / (layer_grad_norm + 1e-8)
#         weight_ratio = weight_tensor / (layer_weight_norm + 1e-8)
#         importance_score = grad_ratio / (weight_ratio + 1e-8)  # (param_grad / layer_grad_norm) / (param_weight / layer_weight_norm)
...
# class GradWeightlnSelectMasking:
...
#                         GradWeightlnSelectMaskingLinear(base_Linear=module, masking_prob=self.masking_prob))
...
#         layerwise_ratios = {}
#         total_grad_weight_norm = 0.0 

#         for name, module in self.model.named_modules():
#             if isinstance(module, GradWeightlnSelectMaskingLinear) and any(layer_name in name for layer_name in self.linear_module_list):
#                 grad_key = self._normalize_key(name + ".weight")
#                 weight_key = self._normalize_key(name + ".weight")

#                 grad_tensor = self.gradients.get(grad_key, None).abs()
#                 weight_tensor = self.weights.get(weight_key, None).abs()

#                 if grad_tensor is not None and weight_tensor is not None:
#                     grad_norm = grad_tensor.norm().item() 
#                     weight_norm = weight_tensor.norm().item() 
...
#                     if weight_norm > 0:
#                         layerwise_ratio = grad_norm / weight_norm
#                     else:
#                         layerwise_ratio = 0.0
#                     print(f'--- name: {name} layerwise ratio: {layerwise_ratio}, grad norm: {grad_norm}, weight norm: {weight_norm}')

#                     layerwise_ratios[name] = layerwise_ratio
#                     total_grad_weight_norm += layerwise_ratio

#         if total_grad_weight_norm == 0.0:
#             logger.warning("Total gradient/weight norm sum is 0. Skipping masking.")
#             return
...
#                 layer_ratio = layerwise_ratios[name] / total_grad_weight_norm  
...
#                     layer_grad_norm = grad.norm()
#                     layer_weight_norm = weight.norm()

#                     module.apply_mask(self.mask_mode, grad, weight, layer_grad_norm, layer_weight_norm, n_tuning_params)
...
class GradWeightlnSelectMasking:
    ...
    def apply_grad_weight_select_masking(self):
        ...
        # Apply the mask using the selected layer ratios
        for name, module in self.model.named_modules():
            if name in layerwise_ratios:
