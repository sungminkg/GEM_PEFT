...
# class SaveWeightForCallback(TrainerCallback):
#     def __init__(self, save_epochs, layer_keywords, save_dir="parameters"):
...
#         folder_name = os.path.join(self.save_dir, task, tag)
#         os.makedirs(folder_name, exist_ok=True)
#         return folder_name

#     def _save_selected_weights_and_grads(self, model, args, epoch_name):
...
#                 selected_weights[name] = param.detach().cpu()
#                 if name in self.gradient_buffer:
#                     selected_grads[name] = self.gradient_buffer[name].detach().cpu()
#                 else:
#                     print(f"[CALLBACK] ⚠️ No gradient for {name} at {epoch_name}")

#         torch.save(selected_weights, os.path.join(save_path, f"{epoch_name}_layer_weights.pt"))
#         torch.save(selected_grads, os.path.join(save_path, f"{epoch_name}_layer_grads.pt"))
#         print(f"[CALLBACK] ✅ Saved weights + grads for {epoch_name}")
...
#                 model = kwargs['model']
#                 for name, param in model.named_parameters():
#                     if any(k in name for k in self.layer_keywords) and param.grad is not None:
#                         self.gradient_buffer[name] = param.grad.clone().detach()

#     def on_epoch_end(self, args, state, control, **kwargs):
#         if self.current_epoch in self.save_epochs:
#             model = kwargs['model']
#             self._save_selected_weights_and_grads(model, args, epoch_name=f"epoch{self.current_epoch}")

#     def on_train_begin(self, args, state, control, **kwargs):
#         if not self.initial_saved:
#             model = kwargs['model']
#             self._save_selected_weights_and_grads(model, args, epoch_name="epoch0")
#             self.initial_saved = True


# class MyTrainer(Trainer):
#     def __init__(self, *args, save_grad_epochs=None, save_dir=None, layer_keywords=None, **kwargs):
#         super().__init__(*args, **kwargs)
#         self.save_grad_epochs = save_grad_epochs or []
#         self.save_dir = save_dir or "./saved_grads"
#         self.layer_keywords = layer_keywords or []

#         os.makedirs(self.save_dir, exist_ok=True)

#     def training_step(self, model, inputs, num_inputs=None):
#         model.train()
...
#         if int(self.state.epoch) in self.save_grad_epochs:
#             self.save_gradients(model, int(self.state.epoch))

#         return loss.detach()



#     def save_gradients(self, model, epoch):
#         task_name = getattr(self.args, 'task_name', 'unknown_task')
#         tag = getattr(self.args, 'tag', 'default')
#         save_path = os.path.join(self.save_dir, task_name, tag)
...
#         for name, param in model.named_parameters():
#             if any(kw in name for kw in self.layer_keywords):
#                 if param.grad is not None:
#                     grad_dict[name] = param.grad.detach().cpu()
#                     weight_dict[name] = param.detach().cpu()
#                 else:
#                     print(f"[SKIP] No grad for {name} at epoch {epoch}")

#         # Save only once after all gradients collected
#         grad_path = os.path.join(save_path, f"epoch{epoch}_layer_grads.pt")
#         weight_path = os.path.join(save_path, f"epoch{epoch}_layer_weights.pt")
#         torch.save(grad_dict, grad_path)
#         torch.save(weight_dict, weight_path)
#         print(f"[SAVED] Gradients + Weights saved at epoch{epoch} ✅")
...
#                                                             pad_to_multiple_of=8) if self.args.train_as_classification else collator(
#                 self.tokenizer, pad_to_multiple_of=8),
#             save_grad_epochs=[1, 2],  # 저장하고 싶은 epoch
#             save_dir="./parameters",  # 저장 경로
#             layer_keywords=["q_proj", "v_proj"]  # 저장할 레이어 키워드
#         )
