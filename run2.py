...
os.environ["TMPDIR"] = "/vault/tmp/kangsung"
import logging
import argparse
import time
import re
...
import csv
import shutil
from transformers import AutoConfig, AutoTokenizer, AutoModelForCausalLM, HfArgumentParser, GenerationConfig, \
    TrainingArguments, DataCollatorWithPadding, DataCollatorForTokenClassification, Trainer, TrainerCallback, \
    Seq2SeqTrainer, Seq2SeqTrainingArguments, AutoModelForSequenceClassification, AutoModelForQuestionAnswering, AutoModelForMultipleChoice
import torch
...
@dataclass
class OurArguments(Seq2SeqTrainingArguments):   # TrainingArguments, Seq2SeqTrainingArguments
    ...
    max_length: int = 1024  # max length the model can take
    ...
    adalora_alpha: int = 16            # same meaning as lora_alpha
    adalora_r: int = 12                # initial max rank
    adalora_target_rank: int = 6       # final target rank after scheduling
    total_step: int = 1000             # total training steps (used for scheduling)
...
class SaveWeightForSVDCallback(TrainerCallback):
    def __init__(self, save_epochs, layer_keywords, save_dir="svd_weights", framework=None):  # 👈 여기 framework 추가
    ...
    def _make_save_path(self, args):
        ...
        save_path = os.path.join(self.save_dir, task, tag)
    ...
    def _save_selected_weights(self, model, args, epoch_name):
        ...
        selected_weights = {
            ...
            if any(k in name for k in self.layer_keywords)
        }
    ...
    def _save_selected_gradients(self, gradients, args, epoch_name):
        save_path = self._make_save_path(args)
        selected_gradients = {
            name: grad.cpu()
        ...
        torch.save(selected_gradients, os.path.join(save_path, f"{epoch_name}_layer_gradients.pt"))
        print(f"[SVD-CALLBACK] Saved gradients: {epoch_name}")

    def on_epoch_end(self, args, state, control, **kwargs):
        current_epoch = int(state.epoch)
        if current_epoch in self.save_epochs:
            model = kwargs['model']
            self._save_selected_weights(model, args, f"epoch{current_epoch}")


# AdaLoRA parameter compute
from transformers import TrainerCallback, TrainerControl, TrainerState
from typing import Any, Dict

class AdaLoRAAdaptiveRankCallback(TrainerCallback):
    def on_step_end(self, args, state: TrainerState, control: TrainerControl, **kwargs):
        model = kwargs["model"]
        for _, module in model.named_modules():
            if isinstance(module, AdaLoRALinear):
                # Adaptive masking
                module.update_importance_score()
                module.adjust_rank()

                # Rank usage tracking
                if not hasattr(module, "rank_accumulator"):
                    module.rank_accumulator = 0.0
                    module.rank_step_counter = 0
                module.rank_accumulator += module.mask.sum().item()
                module.rank_step_counter += 1
    ...
    def on_train_end(self, args, state: TrainerState, control: TrainerControl, **kwargs):
        ...
        for _, module in model.named_modules():
            if isinstance(module, AdaLoRALinear):
                avg_r = module.rank_accumulator / (module.rank_step_counter + 1e-6)
                total_tunable += 2 * avg_r * hidden_dim
                total_steps = max(total_steps, module.rank_step_counter)
        total_params = sum(p.numel() for p in model.parameters())
        percent = 100 * total_tunable / total_params
        print(f"[AdaLoRA] Average tuned param usage: {percent:.4f}% over {total_steps} steps")
...
class Framework:
    ...
    def backward_qv_pass(self):
        ...
        Computes gradients for entire model (to preserve autograd graph for checkpointing),
        but only stores gradients for 'q_proj' and 'v_proj' layers.
        """
        ...
        # 1️⃣ 전체 모델 requires_grad True → graph가 끊기지 않게 함
        ...
        for step, batch in enumerate(tqdm(dataloader, desc="Backward Pass")):
            ...
            # 2️⃣ 여기서 selective로 q_proj / v_proj 만 gradient 저장
        ...
        # 3️⃣ 끝나고 다시 selective freezing (선택사항)
        for name, param in self.model.named_parameters():
    ...
    def load_model(self):
        if self.args.lora:
            ...
            AdaLoRA(
                self.model,
                r=self.args.adalora_r,
                alpha=self.args.adalora_alpha,
                total_step=self.args.total_step,
                target_rank=self.args.adalora_target_rank
            )
            ...
            # torch.save(gradients, "parameters/opt1.3b_gradients.pt", _use_new_zipfile_serialization=False)
            # torch.save({k: v.clone().cpu() for k, v in self.model.named_parameters()}, 
            #            "parameters/opt1.3b_pretrained_weights.pt", _use_new_zipfile_serialization=False)
            ...
            for param in self.model.parameters():
                param.requires_grad = True
            self.gradients = self.backward_qv_pass()
            return
        ...
        for name, module in self.model.named_modules():
            if hasattr(module, "masking"):
                module.masking = module.masking.to(dtype=module.tunable_weight.dtype, device=module.tunable_weight.device)
    ...
    def evaluate(self, train_samples, eval_samples, one_train_set_per_eval_sample=False):
        ...
        # GSM8k
        # metrics = self.compute_metrics((np.array([p.predicted_candidate for p in predictions]),
        #                                 np.array([p.correct_candidate for p in predictions])))
        # final_accuracy = metrics.get("exact_match", 0.0)
    ...
    #     # ✅ 새로운 clean evaluation args (deepspeed 제거됨)
    ...
    #     output = trainer.predict(eval_samples)
    #     metrics = output.metrics

    #     final_accuracy = metrics.get("eval_exact_match", 0.0)
    #     experiment_results = {
    #         "model": self.args.model_name,
    #         "task": self.args.task_name,
    #         "mode": self.args.tag,
    ...
    # for GSM8k
    def extract_numeric_answer(self, s: str) -> str:
        # 먼저 #### 뒤를 우선 탐색
        if "####" in s:
            after = s.split("####")[-1]
        else:
            # fallback: 끝에서부터 한 문단 정도 추출
            after = s.strip().split('\n')[-1]
    ...
    def compute_metrics(self, eval_preds):
        ...
        if isinstance(predictions[0], (list, np.ndarray)):
            ...
            pred_strs = self.tokenizer.batch_decode(predictions, skip_special_tokens=True)
            label_strs = self.tokenizer.batch_decode(labels, skip_special_tokens=True)
        else:
            pred_strs = predictions
            label_strs = labels

        pred_answers = [self.extract_numeric_answer(p) for p in pred_strs]
        gold_answers = [self.extract_numeric_answer(l) for l in label_strs]
        ...
        exact_match = sum(matches) / len(matches)

        print("Extracted predicted answers:", pred_answers[:3])
        print("Extracted gold answers:", gold_answers[:3])
    ...
    def train(self, train_samples, eval_samples):
        ...
        trainer = Trainer(
            ...
            callbacks=[
                EmptyCacheCallback(),
                # AdaLoRAAdaptiveRankCallback(),    # adalora
        ...
        #     tokenizer=self.tokenizer,
        #     compute_metrics=self.compute_metrics,
        #     data_collator=collator(self.tokenizer, pad_to_multiple_of=8),
